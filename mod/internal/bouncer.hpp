...
struct bouncer_mod : public internal_mod {
    ...
    virtual int mod_signal()
    {
        ...
        // check reserved size depending on version
            front.orders->reserve_order(align4(entry_b->pbmp->cx * nbbytes(entry_b->pbmp->bpp)) * entry_b->pbmp->cy + 16);
        ...
        // check reserved size depending on version
            front.orders->reserve_order(align4(entry_b_h->pbmp->cx * nbbytes(entry_b_h->pbmp->bpp)) * entry_b_h->pbmp->cy + 16);
            bmp.emit(front.orders->out_stream);
        }
        ...
        BitmapCacheItem * entry_b_wallix = front.bmp_cache->get_item(cache_b_id_wallix, cache_b_idx_wallix);
        {
           ...
           // check reserved size depending on version
            front.orders->reserve_order(align4(entry_b_wallix->pbmp->cx * nbbytes(entry_b_wallix->pbmp->bpp)) * entry_b_wallix->pbmp->cy + 16);
            bmp.emit(front.orders->out_stream);
        }
        ...
        // check reserved size depending on version
            front.orders->reserve_order(align4(entry_b_h_wallix->pbmp->cx * nbbytes(entry_b_h_wallix->pbmp->bpp)) * entry_b_h_wallix->pbmp->cy + 16);
            bmp.emit(front.orders->out_stream);
        }
        front.orders->send();
