...
//        LOG(LOG_INFO, "============================== DRAW_EVENT %u =================================", count++);
        ...
        case MOD_RDP_CONNECTION_INITIATION:
            ...
            if (this->verbose){
                LOG(LOG_INFO, "mod_rdp::mcs_send_connect_initial");
            }
            mcs_send_connect_initial(
                    this->trans, this->front.get_channel_list(), this->front_width, this->front_height, this->front_bpp, keylayout, hostname, this->use_rdp5, this->console_session);
            if (this->verbose){
                LOG(LOG_INFO, "mod_rdp::mcs_send_connect_initial done");
            }
        ...
        case MOD_RDP_BASIC_SETTINGS_EXCHANGE:
            if (this->verbose){
                LOG(LOG_INFO, "mod_rdp::mcs_recv_connect_response");
            }
            mcs_recv_connect_response(
                    ...
                    this->gen);
            if (this->verbose){
                LOG(LOG_INFO, "mod_rdp::mcs_recv_connect_response done");
            }
