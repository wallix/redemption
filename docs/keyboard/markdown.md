...
Correspondance de touche physique (scancode) en touche virtuelle (VK) par rapport aux modificateurs activés.
...
Une valeur entre 0 et 127 inclut (`0x7f`) associé à des flags:

--
- Release (`0x8000`): indique que la touche est relachée. Son absence indique que la touche est préssée

--
- Down (`0x4000`): une priorité d'événement (???)
...
- Le layout clavier du client n'est pas connut ou n'a pas d'équivalent, on ne peut pas se baser sur `KeyboardEvent.code` (équivalent scancode).

--

- Mais il faut utilise `KeyboardEvent.code` pour distinguer certaines touches (ctrl droit/gauche, nombre du pavé numérique, etc).

--

- Les modificateurs activés par le client ne sont pas forcément ceux nécessaire pour activer la touche en utilisant un scancode (il faut simuler des frappes).
...
Sauf pour synchroniser au moment de la prise de focus.

--
- `key`: une VK comme (`a`, `A`, `AudioVolumeDown`, `Control`, `Dead`, `4`, etc)

--
- `code`: l'équivalent scancode (`KeyA`, `VolumeDown`, `ControlRight`, `Digit6`, `Numpad4`, etc)
...
--
    - Que faire si `code === 'ControlRight'` mais qu'elle n'exsite pas (remplacée par OEM8) ?

--
- Dans le cas contraire, `key` représente la VK.

--
Note: lorsque que `key.length === 1`, cela représente une lettre
...
--
```rs
convert(key:String, isReleased:bool):Array[Scancode /* uint8_t */]
```
...
- Un même VK peut avoir plusieurs séquence de scancode.
...
--
    - Sinon prend le premier élément puis met dans le tableau de retour
        - La désactivation des modificateurs qui ne sont pas nécessaire
        - L'activation des modificateurs qui manque
        - Le scancode
        - La désactivation des modificateurs ajoutés
        - La réactivation des modificateurs enlevé à l'étape 1
...
À noter que Ctrl ne peut pas être combiné avec un caractère unicode.
