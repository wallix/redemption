...
// a delta byte that indicates the distance between two consecutive fragments; 
// this distance is measured in pixels from the beginning of the first fragment
// to the beginning of the next. If the distance is greater than 127 (0x7F), then
...
//  SO_CHAR_INC_EQUAL_BM_BASE (0x20) flag, then the index byte MUST be followed by
//  a delta byte that indicates the distance between two consecutive glyphs; this
//  distance is measured in pixels from the beginning of the first glyph to the
//  beginning of the next. If the distance is greater than 127 (0x7F), then the
