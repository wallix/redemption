...
//    EncryptedPlatformChallengeResponse (variable): A LICENSE_BINARY_BLOB<14> structure (as specified in [MS-RDPBCGR] section 2.2.1.12.1.2) of wBlobType BB_ENCRYPTED_DATA_BLOB (0x0009). This BLOB contains the encrypted Platform Challenge Response Data (section 2.2.2.5.1) generated by the client and is encrypted with the licensing encryption key (see section 5.1.3), using RC4 (for instructions on how to perform the encryption, see section 5.1.4).
...
//    wClientType (2 bytes): A 16-bit unsigned integer that represents the operating system type of the client and MAY contain one of following values.<15>
...
//0x0001 LICENSE_DETAIL_SIMPLE License Detail Simple (client license certificate and license server certificate without issuer details).

//0x0002 LICENSE_DETAIL_MODERATE License Detail Moderate (client license certificate chain up to license server's certificate issuer).

//0x0003 LICENSE_DETAIL_DETAIL License Detail Detail (client license certificate chain up to root certificate).
