...
struct GraphicDevice
{
    ...
    virtual void draw_window(const Rect & r, uint32_t bgcolor, const char * caption, bool has_focus) = 0;
    virtual void draw_combo(const Rect & r, const char * caption, int state, bool has_focus) = 0;
    virtual void draw_button(const Rect & r, const char * caption, int state, bool has_focus) = 0;
    virtual void draw_edit(const Rect & r, char password_char, char * buffer, size_t edit_pos, bool has_focus) = 0;
    virtual void server_draw_text(uint16_t x, uint16_t y, const char * text, uint32_t fgcolor, uint32_t bgcolor) = 0;
    virtual void opaque_rect(const RDPOpaqueRect & cmd) = 0;
    virtual void scr_blt(const RDPScrBlt & cmd) = 0;
    virtual void dest_blt(const RDPDestBlt & cmd) = 0;
    virtual void pat_blt(const RDPPatBlt & cmd) = 0;
    virtual void mem_blt(const RDPMemBlt & memblt, Bitmap & bitmap, const BGRPalette & palette) = 0;
    virtual void server_set_pen(int style, int width) = 0;

    virtual void line_to(const RDPLineTo & cmd) = 0;
    virtual void glyph_index(const RDPGlyphIndex & cmd) = 0;
    virtual void brush_cache(const int index) = 0;
    ...
    virtual void bitmap_update(Bitmap & bitmap, const Rect & dst, int srcx, int srcy) = 0;
