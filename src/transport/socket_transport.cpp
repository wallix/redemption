...
void SocketTransport::enable_server_tls(const char * certificate_password,
                                        const char * ssl_cipher_list, uint32_t tls_min_level, uint32_t tls_max_level)
{
    if (this->tls != nullptr) {
    ...
    if (!this->tls->enable_server_tls(this->sck, certificate_password, ssl_cipher_list, tls_min_level, tls_max_level)) {
        this->tls.reset();
...
Transport::TlsResult SocketTransport::enable_client_tls(ServerNotifier & server_notifier, uint32_t tls_min_level, uint32_t tls_max_level)
{
    switch (this->tls_state) {
        case TLSState::Uninit:
            LOG(LOG_INFO, "Client TLS start");
            this->tls = std::make_unique<TLSContext>();
            if (!this->tls->enable_client_tls_start(this->sck, this->error_message, tls_min_level, tls_max_level)) {
