...
//        SSHString k_string(len3 + pad3);
        ...
        case SSH_MAC_SHA1:
        {
            ...
            if (this->in_cipher->keysize > this->digest_len * 8) {
                SslSha1 sha1;
                sha1.update(reinterpret_cast<uint8_t const *>(&k_size_n), sizeof(uint32_t));
            ...
            sha1_encryptkey.update(reinterpret_cast<uint8_t const *>(&k_size_n), sizeof(uint32_t));
            sha1_encryptkey.update(&k_string[0], k_string.size());
            sha1_encryptkey.update(this->secret_hash, SHA_DIGEST_LENGTH);
            sha1_encryptkey.update(reinterpret_cast<uint8_t const *>("D"), 1);
            ...
            sha1_decryptMAC.update(reinterpret_cast<uint8_t const *>(&k_size_n), sizeof(uint32_t));
            sha1_decryptMAC.update(&k_string[0], k_string.size());
            sha1_decryptMAC.update(this->secret_hash, SHA_DIGEST_LENGTH);
            sha1_decryptMAC.update(reinterpret_cast<uint8_t const *>("E"), 1);
            ...
            sha1_encryptMAC.update(reinterpret_cast<uint8_t const *>(&k_size_n), sizeof(uint32_t));
            sha1_encryptMAC.update(&k_string[0], k_string.size());
            sha1_encryptMAC.update(this->secret_hash, SHA_DIGEST_LENGTH);
            sha1_encryptMAC.update(reinterpret_cast<uint8_t const *>("F"), 1);
        ...
        case SSH_MAC_SHA256:
        {
            ...
            sha256_decryptIV.update(reinterpret_cast<uint8_t const *>(&k_size_n), sizeof(uint32_t));
            sha256_decryptIV.update(&k_string[0], k_string.size());
            sha256_decryptIV.update(this->secret_hash, SHA256_DIGEST_LENGTH);
            sha256_decryptIV.update(reinterpret_cast<uint8_t const *>("A"), 1);
            ...
            sha256_encryptIV.update(reinterpret_cast<uint8_t const *>(&k_size_n), sizeof(uint32_t));
            sha256_encryptIV.update(&k_string[0], k_string.size());
            sha256_encryptIV.update(this->secret_hash, SHA256_DIGEST_LENGTH);
            sha256_encryptIV.update(reinterpret_cast<uint8_t const *>("B"), 1);
            ...
            sha256_decryptkey.update(reinterpret_cast<uint8_t const *>(&k_size_n), sizeof(uint32_t));
            sha256_decryptkey.update(&k_string[0], k_string.size());
            sha256_decryptkey.update(this->secret_hash, SHA256_DIGEST_LENGTH);
            sha256_decryptkey.update(reinterpret_cast<uint8_t const *>("C"), 1);
            ...
            if (this->in_cipher->keysize > this->digest_len * 8) {
                SslSha256 sha256;
                sha256.update(reinterpret_cast<uint8_t const *>(&k_size_n), sizeof(uint32_t));
            ...
            sha256_encryptkey.update(reinterpret_cast<uint8_t const *>(&k_size_n), sizeof(uint32_t));
            sha256_encryptkey.update(&k_string[0], k_string.size());
            sha256_encryptkey.update(this->secret_hash, SHA256_DIGEST_LENGTH);
            sha256_encryptkey.update(reinterpret_cast<uint8_t const *>("D"), 1);
            ...
            sha256_decryptMAC.update(reinterpret_cast<uint8_t const *>(&k_size_n), sizeof(uint32_t));
            sha256_decryptMAC.update(&k_string[0], k_string.size());
            sha256_decryptMAC.update(this->secret_hash, SHA256_DIGEST_LENGTH);
            sha256_decryptMAC.update(reinterpret_cast<uint8_t const *>("E"), 1);
            ...
            sha256_encryptMAC.update(reinterpret_cast<uint8_t const *>(&k_size_n), sizeof(uint32_t));
    ...
    unsigned char * packet_encrypt(uint32_t seq, uint8_t *data, uint32_t len, error_struct & error) {
        ...
        hmac_sha1.update(Uint32_le(seq).array, sizeof(seq));
