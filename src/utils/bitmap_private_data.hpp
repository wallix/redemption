...
public:
    ...
    static DataBitmap * construct(BitsPerPixel bpp, uint16_t cx, uint16_t cy)
    {
        ...
        return new (p) DataBitmap(bpp, align4(cx), cy, p + sz_struct); /*NOLINT*/
    }
    ...
    void copy_sha1(uint8_t (&sig)[SslSha1::DIGEST_LENGTH]) const
    {
        if (!this->sha1_is_init_) {
            ...
            sha1.update(out_stream.get_produced_bytes());
            ...
            sha1.final(make_writable_sized_array_view(this->sha1_));
        }
