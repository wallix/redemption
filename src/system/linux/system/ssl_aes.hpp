...
class SslAES_direct
{
    ...
    static constexpr int aes_padlock_ace = -1;
...
{                                               \
    ...
    void set_key(uint8_t * e_key, uint8_t * d_key, size_t key_size)
    {
        if ((key_size != 16) &&
            (key_size != 24) &&
            (key_size != 32)) {
        ...
        this->aes_setkey_enc(&this->ctx, e_key,  key_size*8);
        this->aes_setkey_dec(&this->ctx, d_key,  key_size*8);
    }

    void crypt_cbc(size_t data_size, uint8_t * ivec, uint8_t * indata, uint8_t * outdata) {
       this->aes_crypt_cbc(&this->ctx, AES_ENCRYPT, data_size, ivec, outdata, indata);
    }
