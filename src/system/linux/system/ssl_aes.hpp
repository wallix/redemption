...
typedef struct {
      ...
      uint8_t keySched[MAXROUNDS+1][4][MAXBC]; /* key schedule        */
} keyInstance;
...
{
    int direction; // AES_ENCRYPT or AES_DECRYPT
    keyInstance keyInst;
...
private:
    ...
    void Substitution(uint8_t a[4][MAXBC], const uint8_t box[256], uint8_t BC) {
...
int rijndaelKeySched (uint8_t k[4][MAXKC], int keyBits, int blockBits, uint8_t rk[MAXROUNDS+1][4][MAXBC]);
int rijndaelEncrypt (uint8_t a[4][MAXBC], int keyBits, int blockBits, uint8_t rk[MAXROUNDS+1][4][MAXBC]);
int rijndaelEncryptRound (uint8_t a[4][MAXBC], int keyBits, int blockBits, uint8_t rk[MAXROUNDS+1][4][MAXBC], int rounds);
int rijndaelDecrypt (uint8_t a[4][MAXBC], int keyBits, int blockBits, uint8_t rk[MAXROUNDS+1][4][MAXBC]);
int rijndaelDecryptRound (uint8_t a[4][MAXBC], int keyBits, int blockBits, uint8_t rk[MAXROUNDS+1][4][MAXBC], int rounds);
...
int makeKey(keyInstance *key, uint8_t direction, int keyLen, char *keyMaterial)
{
    uint8_t k[4][MAXKC];
    int i, j, t;
...
int blockEncrypt(cipherInstance *cipher,
    keyInstance *key, uint8_t *input, int inputLen, uint8_t *outBuffer)
{
    int i, j, t, numBlocks;
    uint8_t block[4][MAXBC];
...
int cipherUpdateRounds(cipherInstance *cipher,
    keyInstance *key, uint8_t *input, int inputLen, uint8_t *outBuffer, int rounds)
{
    int j, t;
    uint8_t block[4][MAXBC];
...
typedef struct {
      uint8_t  mode;            /* MODE_ECB, MODE_CBC, or MODE_CFB1 */
      uint8_t  IV[MAX_IV_SIZE]; /* A possible Initialization Vector for 
                          ciphering */
      /*  Add any algorithm specific parameters needed here  */
...
int blockEncrypt(cipherInstance *cipher, keyInstance *key, uint8_t *input, 
            int inputLen, uint8_t *outBuffer);

int blockDecrypt(cipherInstance *cipher, keyInstance *key, uint8_t *input,
            int inputLen, uint8_t *outBuffer);
int cipherUpdateRounds(cipherInstance *cipher, keyInstance *key, uint8_t *input, 
                        int inputLen, uint8_t *outBuffer, int Rounds);
