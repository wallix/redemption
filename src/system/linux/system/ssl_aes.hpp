...
enum class AES_direction : uint8_t {
    SSL_AES_ENCRYPT = 0,
    SSL_AES_DECRYPT = 1
};
...
class SslAes128_CBC
{
    ...
    SslAes128_CBC(const uint8_t key[16], const uint8_t (& iv)[16], AES_direction mode)
        : mode(mode==AES_direction::SSL_AES_DECRYPT?AES_DECRYPT:AES_ENCRYPT)
        , key([](const uint8_t key[16], AES_direction mode)-> const AES_KEY {
            AES_KEY tmp_key;
            (mode==AES_direction::SSL_AES_DECRYPT?AES_set_decrypt_key:AES_set_encrypt_key)(key, 128, &tmp_key);
...
class SslAes192_CBC
{
    ...
    SslAes192_CBC(const uint8_t key[24], const uint8_t (& iv)[16], AES_direction mode)
        : mode(mode==AES_direction::SSL_AES_DECRYPT?AES_DECRYPT:AES_ENCRYPT)
        , key([](const uint8_t key[24], AES_direction mode)-> const AES_KEY {
            AES_KEY tmp_key;
            (mode==AES_direction::SSL_AES_DECRYPT?AES_set_decrypt_key:AES_set_encrypt_key)(key, 192, &tmp_key);
            return std::move(tmp_key);
          }(key, mode))
        , tiv([](const uint8_t (& iv)[16])-> const t_iv {
            t_iv tmp;
...
class SslAes256_CBC
{
    ...
    SslAes256_CBC(const uint8_t key[32], const uint8_t (& iv)[16], AES_direction mode)
        : mode(mode==AES_direction::SSL_AES_DECRYPT?AES_DECRYPT:AES_ENCRYPT)
        , key([](const uint8_t key[32], AES_direction mode)-> const AES_KEY {
            AES_KEY tmp_key;
            (mode==AES_direction::SSL_AES_DECRYPT?AES_set_decrypt_key:AES_set_encrypt_key)(key, 256, &tmp_key);
            return std::move(tmp_key);
          }(key, mode))
        , tiv([](const uint8_t (& iv)[16])-> const t_iv {
            t_iv tmp;
...
typedef struct {
      ...
      uint8_t keySched[MAXROUNDS+1][4][BC128]; /* key schedule        */
} keyInstance;
...
private:
    ...
    void Substitution(uint8_t a[4][BC128], const uint8_t box[256]) {
        ...
        for(i = 0; i < 4; i++)
            for(j = 0; j < BC128; j++) a[i][j] = box[a[i][j]] ;
    }
    ...
    int rijndaelDecrypt (uint8_t a[4][BC128], int keyBits, int blockBits, uint8_t rk[MAXROUNDS+1][4][BC128])
    {
    ...
    int rijndaelEncrypt (uint8_t a[4][BC128], int keyBits, int blockBits, uint8_t rk[MAXROUNDS+1][4][BC128])
    {
    ...
    int rijndaelKeySched (uint8_t k[4][KC128], int keyBits, int blockBits, uint8_t W[MAXROUNDS+1][4][BC128]) 
    {
        ...
        for(j = 0; (j < KC128) && (t < (ROUNDS+1)*BC128); j++, t++)
            for(i = 0; i < 4; i++) W[t / BC128][i][t % BC128] = tk[i][j];
...
private:
    ...
    int rijndaelKeySched (uint8_t k[4][KC192], int keyBits, int blockBits, uint8_t W[MAXROUNDS+1][4][BC128]) 
    {
        /* Calculate the necessary round keys
         * The number of calculations depends on keyBits and blockBits
         */
        ...
        t = 0;
        /* copy values into round key array */
        for(j = 0; (j < KC192) && (t < (ROUNDS+1)*BC128); j++, t++){
            for(i = 0; i < 4; i++){
                 W[t / BC128][i][t % BC128] = tk[i][j];
            }
        }
...
private:
    ...
    int rijndaelKeySched (uint8_t k[4][KC256], int keyBits, int blockBits, uint8_t W[MAXROUNDS+1][4][BC128]) 
    {
        /* Calculate the necessary round keys
         * The number of calculations depends on keyBits and blockBits
         */
