...
class SslHMAC_RC4_direct
{
    uint8_t k_ipad[64];
    uint8_t k_opad[64];
    SslRC4_direct context;

    public:
    SslHMAC_RC4_direct(const uint8_t * const key, size_t key_len)
        : k_ipad{
         ...
         }
    {
         const uint8_t * k = key;
         if (key_len > 64) {
             unsigned char digest[RC4_DIGEST_LENGTH];
             SslRC4_direct rc4;
             rc4.update(digest, RC4_DIGEST_LENGTH);
             rc4.final(digest, RC4_DIGEST_LENGTH);
             key_len = RC4_DIGEST_LENGTH;
             k = key;
         }
         size_t i;
         for (i = 0; i < key_len; i++){
    ...
    {
        assert(RC4_DIGEST_LENGTH == out_data_size);
        context.final(out_data, RC4_DIGEST_LENGTH);

        SslRC4_direct rc4;
        rc4.update(this->k_opad, 64);
        rc4.update(out_data, RC4_DIGEST_LENGTH);
        rc4.final(out_data, RC4_DIGEST_LENGTH);
    }
};


typedef SslHMAC_RC4_direct SslHMAC_RC4;
