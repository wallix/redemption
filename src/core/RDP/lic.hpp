...
//            stream.p -= 4;
    ...
    struct ProductInformation
    {
        ...
        array_view<const uint8_t> CompanyName;

        array_view<const uint8_t> ProductId;

        size_t str(char * buffer, size_t sz) const
        {
            uint8_t CompanyNameU8[4096] {};
            ::UTF16toUTF8(CompanyName.data(), CompanyName.size() / sizeof(uint16_t), CompanyNameU8, sizeof(CompanyNameU8));

            uint8_t ProductIdU8[4096] {};
            ::UTF16toUTF8(ProductId.data(), ProductId.size() / sizeof(uint16_t), ProductIdU8, sizeof(ProductIdU8));
    ...
    inline ProductInformation ProductInformation_Receiver(InStream& stream) {
        ...
        ProductInfo.CompanyName = array_view<const uint8_t>(stream.get_current(), cbCompanyName);
        ...
        ProductInfo.ProductId = array_view<const uint8_t>(stream.get_current(), cbProductId);
    ...
    struct LicensingBinaryBlob
    {
        ...
        array_view<const uint8_t> blobData;
    ...
    inline LicensingBinaryBlob LicensingBinaryBlob_Receiver(InStream& stream)
    {
        ...
        LicBinBlob.blobData = array_view<const uint8_t>(stream.get_current(), wBlobLen);
...
//            stream.out_copy_bytes(signature, LIC::LICENSE_SIGNATURE_SIZE);
    ...
    struct NewOrUpgradeLicense_Recv
    {
        ...
        {
            uint8_t CompanyNameU8[4096] {};
            ::UTF16toUTF8(this->licenseInfo.pbCompanyName, this->licenseInfo.cbCompanyName / sizeof(uint16_t), CompanyNameU8, sizeof(CompanyNameU8));

            uint8_t ProductIdU8[4096] {};
            ::UTF16toUTF8(this->licenseInfo.pbProductId, this->licenseInfo.cbProductId / sizeof(uint16_t), ProductIdU8, sizeof(ProductIdU8));
