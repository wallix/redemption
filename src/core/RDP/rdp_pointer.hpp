...
struct NewPointer : public BasePointer {
...
public:
    ...
    explicit NewPointer(const CursorSize & d, const Hotspot & hs, array_view_const_u8 av_xor, array_view_const_u8 av_and, uint8_t data_bpp, const BGRPalette & palette, bool clean_up_32_bpp_cursor, BogusLinuxCursor bogus_linux_cursor)
        : BasePointer(d, hs)
    {
        ...
        if (data_bpp == 1) {
            uint8_t data_data[NewPointer::MAX_WIDTH * NewPointer::MAX_HEIGHT / 8];
            uint8_t mask_data[NewPointer::MAX_WIDTH * NewPointer::MAX_HEIGHT / 8];
    ...
    NewPointer(uint8_t Bpp, CursorSize d, Hotspot hs, const std::vector<uint8_t> & vncdata, const std::vector<uint8_t> & vncmask, 
...
//                LOG(LOG_INFO, "y=%u xx=%u source_mask_offset=%u target_mask_offset=%u")";
    ...
    explicit NewPointer(CursorSize d, Hotspot hs, array_view_const_u8 av_xor, array_view_const_u8 av_and)
        : BasePointer(d, hs)
    {
    ...
    explicit NewPointer(uint8_t data_bpp, CursorSize d, Hotspot hs, array_view_const_u8 av_xor, array_view_const_u8 av_and)
    : BasePointer(d, hs)
    {
        (void)data_bpp;
        if ((av_and.size() > this->bit_mask_size()) || (av_xor.size() > this->xor_mask_size())) {
...
//    }
    ...
    bool operator==(const NewPointer & other) const {
    ...
    explicit NewPointer(const ConstPointer & p = NullPointer{})
...
class ColorPointerUpdate {
    int cache_idx;
    const NewPointer & cursor;

public:
    ColorPointerUpdate(int cache_idx, const NewPointer & cursor)
        : cache_idx(cache_idx)
        , cursor(cursor)
    {
    }
...
public:
    NewPointerUpdate(int cache_idx, const NewPointer & cursor)
        : cache_idx(cache_idx)
        , cursor(cursor)
    {
    }
...
//      bytes).
        ...
        if (this->cursor.only_black_white) {
            uint8_t xorMaskData[NewPointer::MAX_WIDTH * NewPointer::MAX_HEIGHT * 1 / 8] = { 0 };
            ...
            uint8_t andMaskData[NewPointer::MAX_WIDTH * NewPointer::MAX_HEIGHT / 8] = { 0 };
            ...
            uint8_t xorMaskData[NewPointer::MAX_WIDTH * NewPointer::MAX_HEIGHT * 4] = { 0 };
            auto av_xor = this->cursor.get_24bits_xor_mask();
...
struct ARGB32Pointer {
    ...
    alignas(4) 
    uint8_t data[NewPointer::DATA_SIZE];

    ARGB32Pointer(NewPointer const & cursor)
