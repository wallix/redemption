...
struct rdp_mppc_40_dec : public rdp_mppc_dec
{
     ...
     */
    void decompress_40(uint8_t const * cbuf, int len, int ctype, uint32_t * roff, uint32_t * rlen)
    {
    ...
    bytes_view decompress(bytes_view cbuf, int ctype) override
    {
        uint32_t roff   = 0;
        uint32_t rlen   = 0;
        this->decompress_40(cbuf.data(), cbuf.size(), ctype, &roff, &rlen);
...
struct rdp_mppc_40_enc : public rdp_mppc_enc
{
    ...
    uint16_t   outputBufferSize = RDP_40_HIST_BUF_LEN + 8;
