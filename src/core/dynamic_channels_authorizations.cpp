    ...
    std::string const & allow, std::string const & deny)
{
    ...
    this->all_allow = extract(allow, this->allowed_names);
    this->all_deny  = extract(deny, this->denied_names);

    if (this->allowed_names.empty() && !this->all_allow) {
        this->all_deny = true;

        this->denied_names.clear();
    }

    if (this->denied_names.empty() && !this->all_deny) {
        this->all_allow = true;
    ...
    assert((this->all_allow && this->allowed_names.empty()) ||
           !this->all_allow);

    assert((this->all_deny && this->denied_names.empty()) ||
           !this->all_deny);
}
