...
public:
    ...
    virtual void ProcessServerDriveControlRequest(
        ...
        if (bool(verbose & RDPVerboseFlags::fsdrvmgr)) {
            LOG(LOG_INFO,
...
public:
    void AnnounceDrive(bool device_capability_version_02_supported,
            VirtualChannelDataSender& to_server_sender, RDPVerboseFlags verbose) {
...
private:
    uint32_t EnableDrive(const char * drive_name, const char * relative_directory_path,
                         bool read_only, RDPVerboseFlags verbose,
...
public:
    bool EnableDrive(const char * relative_directory_path, RDPVerboseFlags verbose,
    ...
    bool EnableSessionProbeDrive(RDPVerboseFlags verbose) {
...
public:
    ...
    void RemoveSessionProbeDrive(RDPVerboseFlags verbose) {
        if (this->session_probe_drive_id == INVALID_MANAGED_DRIVE_ID) {
            return;
        }
