...
void CopyPaste::paste(WidgetEdit & edit)
{
    if (this->has_clipboard_) {
        ...
        send_to_front_channel_2<RDPECLIP::FormatDataRequestPDU>(
            *this->front_, *this->channel_, RDPECLIP::CB_FORMAT_DATA_REQUEST, RDPECLIP::CB_RESPONSE_OK
...
void CopyPaste::send_to_mod_channel(InStream & chunk, uint32_t flags)
{
    ...
    switch (rp.msgType()) {
        ...
        case RDPECLIP::CB_FORMAT_DATA_RESPONSE: {
            ...
            format_data_response_header.recv(stream);
            ...
            if (format_data_response_header.msgFlags() == RDPECLIP::CB_RESPONSE_OK) {
                ...
                if ((flags & CHANNELS::CHANNEL_FLAG_LAST) != 0) {
                    if (!stream.in_check_rem(format_data_response_header.dataLen())) {
                        LOG( LOG_ERR
                            , "CopyPaste::send_to_mod_channel truncated CB_FORMAT_DATA_RESPONSE dataU16, need=%" PRIu32 " remains=%zu"
                            , format_data_response_header.dataLen(), stream.in_remain());
                        throw Error(ERR_RDP_PROTOCOL);
                    }

                    this->clipboard_str_.utf16_push_back(stream.get_current(), format_data_response_header.dataLen() / 2);
                    ...
                    this->long_data_response_size = format_data_response_header.dataLen() - stream.in_remain();
