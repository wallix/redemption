...
, rail_enabled(client_execute_object.is_rail_enabled())
    ...
    if (this->rail_enabled) {
        ...
        .on_action(jln::one_shot([this](gdi::GraphicApi&){
            if (!this->client_execute_object) {
                this->client_execute_object.ready(
...
LocallyIntegrableMod::~LocallyIntegrableMod()
{
    this->client_execute_object.reset(true);
}
...
void LocallyIntegrableMod::rdp_input_invalidate(Rect r)
{
    ...
    if (this->rail_enabled) {
        this->client_execute_object.input_invalidate(r);
    }
}
...
void LocallyIntegrableMod::rdp_input_mouse(int device_flags, int x, int y, Keymap2 * keymap)
{
    ...
    if (!this->rail_enabled) {
        ...
        if (!this->client_execute_object.input_mouse(device_flags, x, y, out_mouse_captured)) {
            switch (this->dc_state) {
                ...
                case DCState::SecondClickDown:
                    if (device_flags == SlowPath::PTRFLAGS_BUTTON1) {
                        ...
                        this->client_execute_object.input_mouse(PTRFLAGS_EX_DOUBLE_CLICK, x, y, out_mouse_captured_2);
...
{
    if (this->rail_enabled && this->client_execute_object 
    && front_channel_name == CHANNELS::channel_names::rail) {

        this->client_execute_object.send_to_mod_rail_channel(length, chunk, flags);
    }
    else if (this->rail_enabled && this->client_execute_object 
