...
private:
    void check_timeout()
    {
        if (this->clipboard_requesting_for_data_is_delayed) {
            ...
            formatListRequestPDUHeader.emit(out_s);
    ...
    void clipboard_send_to_vnc(InStream & chunk, size_t length, uint32_t flags)
    {
        ...
        switch (msgType) {
            ...
            case RDPECLIP::CB_FORMAT_DATA_REQUEST: {
                ...
                if (bool(this->verbose & VNCVerbose::clipboard)) {
                    LOG( LOG_INFO
                       ...
                       , RDPECLIP::CB_FORMAT_DATA_REQUEST
                       , formatListRequestPDUHeader.msgFlags()
                       , formatListRequestPDUHeader.dataLen()
                ...
                if (this->bogus_clipboard_infinite_loop != VncBogusClipboardInfiniteLoop::delayed && this->clipboard_owned_by_client) {
                    ...
                    const RDPECLIP::CliprdrHeader formatDataResponseHeader(RDPECLIP::CB_FORMAT_DATA_RESPONSE, RDPECLIP::CB_RESPONSE_OK, this->to_vnc_clipboard_data.get_offset()+4);
            ...
            case RDPECLIP::CB_FORMAT_DATA_RESPONSE: {
                RDPECLIP::CliprdrHeader format_data_response_header;

                format_data_response_header.recv(chunk);

                RDPECLIP::FormatDataResponsePDU format_data_response_pdu;

                format_data_response_pdu.recv(chunk);

                if (format_data_response_header.msgFlags() == RDPECLIP::CB_RESPONSE_OK) {
                    if ((flags & CHANNELS::CHANNEL_FLAG_LAST) != 0) {
                        if (!chunk.in_check_rem(format_data_response_header.dataLen())) {
                            LOG( LOG_ERR
                               , "mod_vnc::clipboard_send_to_vnc: truncated CB_FORMAT_DATA_RESPONSE(%u), need=%u remains=%zu"
                               , RDPECLIP::CB_FORMAT_DATA_RESPONSE
                               , format_data_response_header.dataLen(), chunk.in_remain());
                            throw Error(ERR_VNC);
                        }
                        ...
                        this->to_vnc_clipboard_data.out_copy_bytes(
                                    chunk.get_current(), format_data_response_header.dataLen());
                        ...
                        this->to_vnc_clipboard_data_remaining = format_data_response_header.dataLen();
