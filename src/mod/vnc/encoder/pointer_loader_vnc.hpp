...
inline Pointer pointer_loader_vnc(
    BytesPerPixel Bpp,
    ...
    unsigned blue_shift, unsigned blue_max)
{
        ...
        [&](uint8_t (&data)[Pointer::DATA_SIZE], uint8_t * mask)
        {
            ...
            for (size_t y = 0; y < minheight; ++y) {
                for (size_t x = 0; x < minwidth; ++x) {
                    ...
                    data[target_offset  ] = uint8_t((red   << 3) | (red   >> 2));
                    data[target_offset+1] = uint8_t((green << 2) | (green >> 4));
                    data[target_offset+2] = uint8_t((blue  << 3) | (blue  >> 2));
                }
