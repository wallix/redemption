...
private:
    ...
    static void io_uint32_le(InStream & stream, RDPColor & value) { value = RDPColor::from(BGRColor(stream.in_uint32_le()).as_u32()); }
    static void io_uint32_le(OutStream & stream, RDPColor value) { stream.out_uint32_le(value.as_bgr().as_u32()); }
    ...
    static void io_color(InStream & stream, RDPColor & color) {
        ...
        uint8_t const green = stream.in_uint8();
        uint8_t const blue  = stream.in_uint8();
        color = RDPColor::from(BGRColor(blue, green, red).as_u32());
    }
