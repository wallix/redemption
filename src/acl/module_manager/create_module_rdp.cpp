    ...
    std::array<uint8_t, 28>& server_auto_reconnect_packet)
{
    ...
    mod_rdp_params.adjust_performance_flags_for_recording
                                                        = (ini.get<cfg::globals::is_rec>() &&
                                                            ...
                                                            CaptureFlags::none));
    mod_rdp_params.client_execute_object               = &client_execute_object;
    
    // TODO: add a method to ClientExecute returning a WindowsExecuteShellParams structure
    mod_rdp_params.client_execute.flags                = client_execute_object.Flags();
    mod_rdp_params.client_execute.exe_or_file          = client_execute_object.ExeOrFile();
    mod_rdp_params.client_execute.working_dir          = client_execute_object.WorkingDir();
    mod_rdp_params.client_execute.arguments            = client_execute_object.Arguments();

    mod_rdp_params.should_ignore_first_client_execute  = client_execute_object.should_ignore_first_client_execute();
    ...
    try {
        ...
        if (host_mod_in_widget) {
            ...
            client_execute_object.reset(false);
        }
        ...
        if (host_mod_in_widget) {
            ...
            this->set_mod(host_mod, nullptr, &client_execute_object);
