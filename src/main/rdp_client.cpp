...
int main(int argc, char** argv)
{
    ...
    SocketTransport mod_trans(
        ...
        std::chrono::milliseconds(1000), std::chrono::milliseconds::zero(), std::chrono::seconds(1),
    ...
    Inifile ini;
    if (!ini_file.empty()) {
        configuration_load(Inifile::ConfigurationHolder{ini}.as_ref(), ini_file.c_str());
    }
