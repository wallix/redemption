    ...
    void reset()
    {
        ...
        if (this->mppc_enc) {
            this->max_data_block_size = std::min<size_t>(MAX_DATA_BLOCK_SIZE,
                this->mppc_enc->get_max_data_block_size() - SERIALIZER_HEADER_SIZE);
        }
...
//                        this->process_data(sctrl.payload, cb);
            ...
            // between client-side plug-ins and server-side applications).
            {
                if (buf.current_pdu_is_fast_path()) {
                    ...
                    else if (buf.current_pdu_get_type() != X224::DT_TPDU) {
...
//            front_bitmap_size);
        ...
        if (front_bitmap_size <= max_bmp_size) {
            ...
            const uint16_t TILE_CX = ((::nbbytes(this->client_info.bpp) * 64 * 64 < max_bmp_size) ? 64 : 32);
