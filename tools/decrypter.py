...
GETTRACEKEY = ctypes.CFUNCTYPE(ctypes.c_char_p, ctypes.c_char_p, ctypes.c_int, ctypes.c_void_p)
...
try:
    ...
    libreddec.decmemcpy.argtypes = [ctypes.c_void_p, ctypes.c_char_p]
    libreddec.decmemcpy.restype = ctypes.c_int

    import sys
    myargv = ctypes.c_char_p * len(sys.argv)
    argv = myargv(*tuple(sys.argv))
    libreddec.do_main(len(sys.argv), argv, get_hmac_key_func, get_trace_key_func)
except Exception, e:
    import traceback
