...
def check_po_dir_path(po_dir_path: str) -> None:
    if not os.path.exists(po_dir_path):
        raise OSError(f"missing '{po_dir_path}' directory")  # noqa: EM102
    if not os.path.isdir(po_dir_path):
        raise OSError(f"cannot use '{po_dir_path}' as directory")  # noqa: EM102

def check_msg_catalog_template_path(po_dir_path: str) -> None:
    msg_catalog_template_file = f'{po_dir_path}/{DOMAIN_NAME}.pot'

    if not os.path.exists(msg_catalog_template_file):
        raise OSError(f"missing '{msg_catalog_template_file}' file")  # noqa: EM102
    if not os.path.isfile(msg_catalog_template_file):
        raise OSError(f"cannot use '{msg_catalog_template_file}' as file")  # noqa: EM102

def check_target_locale_dir_path(target_locale_dir_path: str) -> None:
    if not os.path.exists(target_locale_dir_path):
        raise OSError(f"missing '{target_locale_dir_path}' directory")  # noqa: EM102
    if not os.path.isdir(target_locale_dir_path):
        raise OSError(f"cannot use '{target_locale_dir_path}' as directory")  # noqa: EM102

def check_transl_msg_catalog_path(target_locale_dir_path: str) -> None:
    transl_msg_catalog_file = f'{target_locale_dir_path}/{DOMAIN_NAME}.po'

    if not os.path.exists(transl_msg_catalog_file):
        raise OSError(f"missing '{transl_msg_catalog_file}' file")  # noqa: EM102
    if not os.path.isfile(transl_msg_catalog_file):
        raise OSError(f"cannot use '{transl_msg_catalog_file}' as file")  # noqa: EM102



def gen_msg_catalog_template(po_path: str) -> None:
    po_dir_path = f'{po_path}/{PO_OUTPUT_DIRNAME}'

    if not os.path.exists(po_dir_path):
        os.mkdir(po_dir_path)
        print(f"'{po_dir_path}' directory has been generated")
    else:
    ...
    list_xgettext_cmd = [
        ...
        "--c++",
        "--sort-by-file",
        f"--output={po_dir_path}/{DOMAIN_NAME}.pot",
    ...
    print(f"'{po_dir_path}/{DOMAIN_NAME}.pot' file has been generated")

def gen_transl_msg_catalog(target_locale: str, po_path: str) -> None:
    po_dir_path = f'{po_path}/{PO_OUTPUT_DIRNAME}'

    check_po_dir_path(po_dir_path)
    check_msg_catalog_template_path(po_dir_path)

    target_locale_dir_path = f'{po_dir_path}/{target_locale}'

    if not os.path.exists(target_locale_dir_path):
        os.mkdir(target_locale_dir_path)
        print(f"'{target_locale_dir_path}' directory has been generated")
    else:
    ...
    list_msginit_cmd = [
        ...
        f"--input={po_dir_path}/{DOMAIN_NAME}.pot",
        f"--output={target_locale_dir_path}/{DOMAIN_NAME}.po"]

    if subprocess.call(list_msginit_cmd,
                       shell=False,
                       stdout=subprocess.DEVNULL,
                       stderr=subprocess.STDOUT) != 0:
        raise OSError(f"error during '{target_locale}' message catalog generation")  # noqa: EM102

    print(f"'{target_locale_dir_path}/{DOMAIN_NAME}.po' file has been generated")

def gen_modified_transl_msg_catalog_from_tpl(target_locale: str, po_path: str) -> None:
    po_dir_path = f'{po_path}/{PO_OUTPUT_DIRNAME}'
    ...
    list_msgmerge_cmd = [
        ...
        "--force-po",
        "--no-wrap",
        f'{target_locale_dir_path}/{DOMAIN_NAME}.po',
        f'{po_dir_path}/{DOMAIN_NAME}.pot'
    ]
    ...
    if subprocess.call(list_msgmerge_cmd,
                       ...
                       stdout=subprocess.DEVNULL,
                       stderr=subprocess.STDOUT) != 0:
        raise OSError(f"error during '{target_locale}' message catalog modification "
                      "(maybe in your .po file, charset is wrong)")  # noqa: EM102

    print(f"'{target_locale_dir_path}/{DOMAIN_NAME}.po' file has been modified")

def gen_compiled_msg_catalog(target_locale: str, po_path: str, mo_path: str) -> None:
    po_dir_path = f'{po_path}/{PO_OUTPUT_DIRNAME}'

    check_po_dir_path(po_dir_path)

    po_target_locale_dir_path = f'{po_dir_path}/{target_locale}'
    ...
    if not os.path.exists(mo_path):
        os.mkdir(mo_path)
        print(f"'{mo_path}' directory has been generated")
    elif not os.path.isdir(mo_path):
        raise OSError(f"cannot use '{mo_path}' as directory")  # noqa: EM102

    mo_target_locale_dir_path = f'{mo_path}/{target_locale}'

    if not os.path.exists(mo_target_locale_dir_path):
        os.mkdir(mo_target_locale_dir_path)
        print(f"'{mo_target_locale_dir_path}' directory has been generated")
    elif not os.path.isdir(mo_target_locale_dir_path):
        raise OSError(f"cannot use '{mo_target_locale_dir_path}' as directory")  # noqa: EM102

    mo_category_dir_path = f'{mo_target_locale_dir_path}/{MO_CATEGORY_DIRNAME}'

    if not os.path.exists(mo_category_dir_path):
        os.mkdir(mo_category_dir_path)
        print(f"'{mo_category_dir_path}' directory has been generated")
    elif not os.path.isdir(mo_category_dir_path):
        raise OSError(f"cannot use '{mo_category_dir_path}' as directory")  # noqa: EM102

    list_msgfmt_cmd = [
        "msgfmt",
        f"--output-file={mo_category_dir_path}/{DOMAIN_NAME}.mo",
        f"{po_target_locale_dir_path}/{DOMAIN_NAME}.po"
    ]
    ...
    if subprocess.call(list_msgfmt_cmd,
                       ...
                       stdout=subprocess.DEVNULL,
                       stderr=subprocess.STDOUT) != 0:
        raise OSError(f"error during compiled message catalog generation in '{target_locale}' locale")  # noqa: EM102

    print(f"'{mo_category_dir_path}/{DOMAIN_NAME}.mo' file has been generated")

def gen_all_modified_transl_msg_catalogs_from_tpl(po_path: str) -> None:
    po_dir_path = f'{po_path}/{PO_OUTPUT_DIRNAME}'
    ...
    target_locale_dir_name_list = ([target_locale_dir_name
        ...
        if os.path.isdir(f'{po_dir_path}/{target_locale_dir_name}')])

    if not target_locale_dir_name_list:
        raise RuntimeError(f"no locale directory in '{po_dir_path}' directory")  # noqa: EM102

    for target_locale_dir_name in target_locale_dir_name_list:
        gen_modified_transl_msg_catalog_from_tpl(target_locale_dir_name,
                                                 po_path)

def gen_all_compiled_msg_catalogs(po_path: str, mo_path: str) -> None:
    po_path_dir = f'{po_path}/{PO_OUTPUT_DIRNAME}'

    check_po_dir_path(po_path_dir)

    target_locale_dir_name_list = ([target_locale_dir_name
        for target_locale_dir_name in os.listdir(po_path_dir)
        if os.path.isdir(f'{po_path_dir}/{target_locale_dir_name}')])

    if not target_locale_dir_name_list:
        raise RuntimeError(f"no locale directory in '{po_path_dir}' directory")  # noqa: EM102

    for target_locale_dir_name in target_locale_dir_name_list:
        gen_compiled_msg_catalog(target_locale_dir_name, po_path, mo_path)

def execute_generation(args) -> None:
