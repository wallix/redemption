...
}
{
  ...
  if (/^(\x1b\[0;39;49m)?\*\*\* No errors detected$|^\.\.\.skip|^Jamfile|^  *"ccache|^  *"clang|^ *"g\+\+d|^ *em\+\+|\.o\.\.\.$|^In|^  "scan-build/)
  {
    next
  }
  # else if (/[a-z]+\.(compile\.c\+\+|link)|^testing\.unit-test|qt.?\.moc /)
  else if (/^(\x1b\[0;39;49m)?testing\.unit-test |^link(\.dll)?-o |^emscripten|^[-a-zA-Z0-9]+\.(compile\.c\+\+|link|moc)/)
  {
