    ...
    for m in cover.modules:
        ...
        for fnl in sorted(cover.modules[m].functions):
            ...
            if fn.covered_lines == 0:
                print "WARNING: NO COVERAGE %s%s:%s [%s] src/%s/%s" % (td, m, fnl, fn.name, fn.covered_lines, fn.total_lines)
                no_coverage += 1
            elif fn.covered_lines * 100 < fn.total_lines * 50:
                print "WARNING: LOW COVERAGE %s%s:%s [%s] src/%s/%s" % (td, m, fnl, fn.name, fn.covered_lines, fn.total_lines)
                low_coverage += 1
            else:
                print "COVERAGE %s%s:%s [%s] src/%s/%s" % (td, m, fnl, fn.name, fn.covered_lines, fn.total_lines)
                covered += 1
