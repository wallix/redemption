...
void config_spec_definition(Writer && W)
{
    ...
    W.section("globals", [&]
    {
        ...
        W.member(advanced_in_gui | iptables_in_gui | logged_in_gui, no_sesman, L, type_<types::unsigned_>(), names{"port"}, desc{"Warning: Service will be automatically restarted and active sessions will be disconnected\nThe port set in this field must not be already used, otherwise the service will not run.\nChanging the port number will prevent WALLIX Access Manager from working properly."}, set(3389));
