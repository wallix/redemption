...
void config_spec_definition(Writer && W)
{
    ...
    W.section("globals", [&]
    {
        ...
        W.member(hidden_in_gui, rdp_connpolicy, connpolicy::section{"session"}, L, type_<std::chrono::seconds>(), names{"inactivity_timeout"}, desc{"No automatic disconnection due to inactivity, timer is set on target session.\nIf value is between 1 and 30, then 30 is used.\nIf value is set to 0, then value set in \"Session timeout\" (in \"RDP Proxy\" configuration option) is used."}, set(0));
