...
void config_spec_definition(Writer && W)
{
    ...
    W.section(W.names("mod_rdp", connpolicy::name{"rdp"}), [&]
    {
        ...
        W.member(advanced_in_gui, allow_connpolicy_and_gui, rdp_connpolicy, L, type_<ClientAddressSent>(), "client_address_sent", desc { "Enables to set clientAddress field when RDP module is created for info packet" }, set(ClientAddressSent::no_address));
