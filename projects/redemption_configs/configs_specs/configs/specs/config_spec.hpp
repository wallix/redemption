...
void config_spec_definition(Writer && W)
{
    ...
    W.section("globals", [&]
    {
        ...
        W.member(ini_and_gui, no_sesman, L, type_<std::chrono::seconds>(), names{"session_timeout"}, desc{"No traffic auto disconnection, timer is set after authentication on primary session.\nIf value is smaller than 30, 30 is used by default (except 0 which unlimited value is used)."}, set(900));
        W.member(hidden_in_gui, rdp_connpolicy, connpolicy::section{"session"}, L, type_<std::chrono::seconds>(), names{"inactivity_timeout"}, desc{"No traffic auto disconnection, timer is set on secondary session.\nIf value is smaller than 30, 30 is used by default (except 0 which global value \"session_timeout\" is used instead)."}, set(0));
        W.member(advanced_in_gui, no_sesman, L, type_<std::chrono::seconds>(), names{"keepalive_grace_delay"}, desc{"Keepalive."}, set(30));
    ...
    W.section("file_verification", [&]
    {
        ...
        W.member(hidden_in_gui, rdp_connpolicy | advanced_in_connpolicy, L, type_<types::dirpath>(), names{"tmpdir"}, desc{"Temporary path used when files take up too much memory."}, set("/tmp/"));
    });
    ...
    W.section("video", [&]
    {
        ...
        W.member(advanced_in_gui, no_sesman, L, type_<bool>(), names{"bogus_vlc_frame_rate"}, desc{"Needed to play a video with ffplay or VLC.\nNote: Useless with mpv and mplayer."}, set(true));
        ...
        W.member(advanced_in_gui, no_sesman, L, type_<bool>(), names{"play_video_with_corrupted_bitmap"}, desc{"Needed to play a video with corrupted Bitmap Update.\nNote: Useless with mpv and mplayer."}, set(false));
