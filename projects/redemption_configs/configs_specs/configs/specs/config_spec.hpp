...
void config_spec_definition(Writer && W)
{
    ...
    W.section(W.names("mod_rdp", connpolicy::name{"rdp"}), [&]
    {
        ...
        W.member(no_ini_no_gui, rdp_connpolicy, L, type_<std::string>(), "cipher_string", desc{"Tls v1.2 additional ciphers supported by client, default is empty t apply system wide configuration (ssl security level 2), ALL for all ciphers to maximize compatibility (usefull for some WIN2008 servers not up to date)"});
