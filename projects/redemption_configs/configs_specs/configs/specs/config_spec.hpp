...
void config_spec_definition(Writer && W)
{
    ...
    W.section("globals", [&]
    {
        W.member(no_ini_no_gui, proxy_to_sesman, type_<std::chrono::milliseconds>(), "front_connection_time", desc{"from incoming connection to \"up_and_running\" state"});
        W.member(no_ini_no_gui, proxy_to_sesman, type_<std::chrono::milliseconds>(), "target_connection_time", desc{"from Module rdp creation to \"up_and_running\" state"});
        ...
        W.sep();
        W.member(advanced_in_gui, no_sesman, type_<types::dirpath>(), "png_path", set(CPP_EXPR(app_path(AppPath::Png))));
        W.member(advanced_in_gui, no_sesman, type_<types::dirpath>(), "wrm_path", set(CPP_EXPR(app_path(AppPath::Wrm))));
        W.sep();
        ...
        W.member(advanced_in_gui, no_sesman, type_<bool>(), "experimental_support_resize_session_during_recording",set(true));
        W.sep();

        W.member(advanced_in_gui, no_sesman, type_<bool>(), "support_connection_redirection_during_recording",set(true));
        W.sep();
    ...
    W.section("session_log", [&]
    {
        ...
        W.member(hidden_in_gui, sesman_to_proxy, type_<std::string>(), "log_path", sesman::name{"session_log_path"});
        W.sep();
        W.member(hidden_in_gui, sesman_to_proxy, type_<KeyboardInputMaskingLevel>(), "keyboard_input_masking_level", set(KeyboardInputMaskingLevel::password_and_unidentified));
    ...
    W.section("client", [&]
    {
        ...
        W.member(ini_and_gui, no_sesman, type_<uint32_t>(), "tls_min_level", desc{"Minimal incoming TLS level 0=no restriction (TLSv1.0), 1=TLSv1.1, 2=TLSv1.2"}, set(2));
    ...
    W.section("mod_rdp", [&]
    {
        ...
        W.member(hidden_in_gui, sesman_to_proxy, type_<std::chrono::milliseconds>(), "session_probe_clipboard_based_launcher_start_delay", sesman::name{"session_probe_smart_launcher_start_delay"}, set(2000));
        ...
        W.member(advanced_in_gui, no_sesman, type_<bool>(), "split_domain", desc{
    ...
    W.section("mod_vnc", [&]
    {
        ...
        W.member(ini_and_gui, sesman_to_proxy, type_<bool>(), "server_unix_alt", set(false));
    });
    ...
    W.section("video", [&]
    {
        ...
        W.sep();
        W.member(advanced_in_gui, no_sesman, type_<types::dirpath>(), "hash_path", set(CPP_EXPR(app_path(AppPath::Hash))));
        ...
        W.member(ini_and_gui, sesman_to_proxy, type_<KeyboardLogFlags>{}, "disable_keyboard_log", desc{
    ...
    W.section("context", [&]
    {
        ...
        W.member(no_ini_no_gui, sesman_to_proxy, type_<types::range<std::chrono::milliseconds, 3000, 120000>>{}, "rail_disconnect_message_delay", set(3000));
        W.sep();
        ...
        W.member(no_ini_no_gui, sesman_to_proxy, type_<bool>(), "is_wabam", set(false));
        W.sep();
    });
