...
void config_spec_definition(Writer && W)
{
    ...
    W.section("globals", [&]
    {
        ...
        W.member(hidden_in_gui, no_sesman, L, type_<std::chrono::seconds>(), "keepalive_grace_delay", desc{"Internal keepalive between sesman and rdp proxy"}, set(30));
    ...
    {
        W.member(advanced_in_gui, no_sesman, L, type_<types::range<std::chrono::milliseconds, 1000, 10000>>{}, "connection_establishment_timeout", desc{
            "The maximum time in milliseconds that the proxy will wait while attempting to connect to an target."
        }, set(1000));

        W.member(advanced_in_gui, no_sesman, L, type_<types::range<types::u32, 1, 10>>(), "connection_retry_count", desc{
    ...
    W.section(W.names("mod_rdp", connpolicy::name{"rdp"}), [&]
    {
        ...
        W.member(hidden_in_gui, sesman_to_proxy, not_target_ctx, L, type_<types::list<std::string>>(), "allow_channels", desc{"List of enabled (static) virtual channels (example: channel1,channel2,etc). Character * only, activate all with low priority."}, set("*"));
        W.member(hidden_in_gui, sesman_to_proxy, not_target_ctx, L, type_<types::list<std::string>>(), "deny_channels", desc{"List of disabled (static) virtual channels (example: channel1,channel2,etc). Character * only, deactivate all with low priority."});

        W.member(no_ini_no_gui, rdp_connpolicy | advanced_in_connpolicy, L, type_<std::string>(), "allowed_dynamic_channels", desc{"List of enabled dynamic virtual channels (example: channel1,channel2,etc). Character * only, activate all."}, set("*"));
        W.member(no_ini_no_gui, rdp_connpolicy | advanced_in_connpolicy, L, type_<std::string>(), "denied_dynamic_channels", desc{"List of disabled dynamic virtual channels (example: channel1,channel2,etc). Character * only, deactivate all."});
        ...
        W.member(hidden_in_gui, rdp_connpolicy | advanced_in_connpolicy, co_probe, L, type_<bool>(), "session_probe_clipboard_based_launcher_enable_wabam_affinity", connpolicy::name{"smart_launcher_enable_wabam_affinity"}, set(true));
