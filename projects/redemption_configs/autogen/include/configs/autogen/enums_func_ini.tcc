...
inline constexpr zstring_view enum_zstr_VideoEnhancedMode[] {
    "Off"_zv,
    "v1"_zv,
};

inline constexpr zstring_view enum_zint_VideoEnhancedMode[] {
    "0"_zv,
    "1"_zv,
};
...
zstring_view assign_zbuf_from_cfg(
    ...
    (void)zbuf;
    assert(is_valid_enum_value<VideoEnhancedMode>::is_valid(uint8_t(x)));
    return enum_zint_VideoEnhancedMode[uint8_t(x)];
}
...
zstring_view assign_zbuf_from_cfg(
    ...
    (void)zbuf;
    assert(is_valid_enum_value<VideoEnhancedMode>::is_valid(uint8_t(x)));
    return enum_zstr_VideoEnhancedMode[uint8_t(x)];
}

parse_error parse_from_cfg(VideoEnhancedMode & x, ::configs::spec_type<VideoEnhancedMode> /*type*/, bytes_view value)
{
    using ul = uint8_t;
    ...
    x = static_cast<VideoEnhancedMode>(xi);
    return no_parse_error;
}


inline constexpr std::pair<chars_view, VideoEnhancedMode> enum_str_value_VideoEnhancedMode[] {
    {"OFF"_av, VideoEnhancedMode::Off},
    {"V1"_av, VideoEnhancedMode::v1},
};

parse_error parse_from_cfg(VideoEnhancedMode & x, ::configs::spec_type<std::string> /*type*/, bytes_view value)
{
    return parse_str_value_pairs<enum_str_value_VideoEnhancedMode>(
        x, value, "bad value, expected: Off, v1");
}
