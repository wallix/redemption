...
namespace cpp_enumeration_writer
{
    ...
    inline void write_utility_ini(std::ostream & out, type_enumerations const & enums, bool is_hpp)
    {
        ...
        auto parse_fmt = [&](auto & e, auto lazy_integral_parse_fmt){
            ...
            write(e,
                "parse_error parse_from_cfg(%e & x, ::configs::spec_type<%e> /*type*/, chars_view value)\n"
                "{\n"
            );
...
{
    if (ac < 4) {
        std::cerr << av[0] << " type.h utility-ini.h utility-ini.cpp\n";
        return 1;
    }
    ...
    sw.then(av[1], &cpp_enumeration_writer::write_type)
      .then(av[2], &cpp_enumeration_writer::write_utility_ini_hpp)
      .then(av[3], &cpp_enumeration_writer::write_utility_ini_cpp)
    ;
    if (sw.err) {
