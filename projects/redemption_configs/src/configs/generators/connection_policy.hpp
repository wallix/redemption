...
struct ConnectionPolicyWriterBase : python_spec_writer::IniPythonSpecWriterBase
{
    ...
    : python_spec_writer::IniPythonSpecWriterBase(sesman_map_filename)
    ...
    void evaluate_member(std::string const & section_name, Pack const& infos, type_enumerations& enums)
    {
        if constexpr (is_convertible_v<Pack, connection_policy_t>) {
            ...
            python_spec_writer::write_type(this->out(), enums, type, get_default<connpolicy::default_>(type, infos));
            this->out() << "\n\n";
...
vault_transformation_rule = string(default='')
            ...
            if (!out_spec.flush()) {
                std::cerr << "ConnectionPolicyWriterBase: " << filename << ": " << strerror(errno) << "\n";
                return 1;
            }
        }
        ...
        if (!out_sesman) {
            std::cerr << "ConnectionPolicyWriterBase: " << this->filename_ << ": " << strerror(errno) << "\n";
            return 1;
        }
        return 0;
    }
