...
RED_AUTO_TEST_CASE(TestModExp)
{
    ...
    // 12^4 % 3 = 20736 % 3 = 0
    RED_CHECK(mod_exp(writable_array_view{out}, inr, modulus, exponent) == ""_av);

    // 12^4 % 5 = 20736 % 5 = 1
    RED_CHECK(mod_exp(writable_array_view{out}, inr, "\x5"_av, exponent) == "\x1"_av);

    // 12^4 % 17 = 20736 % 17 = 13
    RED_CHECK(mod_exp(writable_array_view{out}, inr, "\x11"_av, exponent) == "\xd"_av);
}
...
RED_AUTO_TEST_CASE(TestBigModExp)
{
    ...
    RED_CHECK(mod_exp(writable_array_view{out}, inr, modulus, exponent) ==
