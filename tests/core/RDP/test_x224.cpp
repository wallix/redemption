...
/* 0050 */ "\x00\x00\x00\x00\x00" //.....
    ...
    X224::CR_TPDU_Data x224 = X224::CR_TPDU_Data_Recv(stream, true);
    ...
    RED_CHECK_EQUAL("Cookie: mstshash=jbberthelin\x0D\x0A"sv, x224.cookie.data);
...
RED_AUTO_TEST_CASE(TestReceive_CR_TPDU_no_factory)
{
    ...
    X224::RecvFactory fac_x224(t, &end, array_size);
    InStream stream({array, std::size_t(end - array)});
    auto x224 = X224::CR_TPDU_Data_Recv(stream, false);

    RED_CHECK_EQUAL(3, x224.tpkt.version);
    RED_CHECK_EQUAL(11, x224.tpkt.len);
    RED_CHECK_EQUAL(static_cast<uint8_t>(X224::CR_TPDU), x224.header.code);
    RED_CHECK_EQUAL(6, x224.header.LI);
    RED_CHECK_EQUAL(0, strlen(x224.cookie.data));
...
RED_AUTO_TEST_CASE(TestReceive_TPDU_truncated_header)
{
    ...
    X224::RecvFactory fac_x224(t, &end, array_size);
    InStream stream({array, std::size_t(end - array)});
    RED_CHECK_EXCEPTION_ERROR_ID(X224::CR_TPDU_Data_Recv(stream, false), ERR_X224);
}
...
RED_AUTO_TEST_CASE(TestSend_CR_TPDU)
{
    ...
    RED_CHECK(stream.get_produced_bytes() == "\x03\x00\x00\x0B\x06\xE0\x00\x00\x00\x00\x00"_av);
}
...
/* 0030 */ "\x00\x08\x00\x01\x00\x00\x00"                                     //....... |
    ...
    RED_CHECK_EQUAL(tpkt_len, x224.tpkt.len);
    RED_CHECK_EQUAL(static_cast<uint8_t>(X224::CR_TPDU), x224.header.code);
    RED_CHECK_EQUAL(0x32, x224.header.LI);

    RED_CHECK_EQUAL("Cookie: mstshash=administrateur@qa\x0D\x0A"sv, x224.cookie.data);
    RED_CHECK_EQUAL(static_cast<uint8_t>(X224::RDP_NEG_REQ), x224.rdp_neg_type);
...
RED_AUTO_TEST_CASE(TestSend_CR_TPDU_TLS_Negotiation_packet)
{
    ...
    RED_CHECK(stream.get_produced_bytes() ==
/* 0000 */ "\x03\x00\x00\x37\x32\xe0\x00\x00\x00\x00\x00\x43\x6f\x6f\x6b\x69" //...72......Cooki |
/* 0010 */ "\x65\x3a\x20\x6d\x73\x74\x73\x68\x61\x73\x68\x3d\x61\x64\x6d\x69" //e: mstshash=admi |
/* 0020 */ "\x6e\x69\x73\x74\x72\x61\x74\x65\x75\x72\x40\x71\x61\x0d\x0a\x01" //nistrateur@qa... |
/* 0030 */ "\x00\x08\x00\x01\x00\x00\x00"                                     //....... |
...
RED_AUTO_TEST_CASE(TestSend_CC_TPDU)
{
    ...
    RED_CHECK(stream.get_produced_bytes() == "\x03\x00\x00\x0B\x06\xD0\x00\x00\x00\x00\x00"_av);
}
...
RED_AUTO_TEST_CASE(TestSend_DR_TPDU)
{
    ...
    RED_CHECK(stream.get_produced_bytes() == "\x03\x00\x00\x0B\x06\x80\x00\x00\x00\x00\x00"_av);
}
...
RED_AUTO_TEST_CASE(TestSend_ER_TPDU)
{
    ...
    RED_CHECK(stream.get_produced_bytes() == "\x03\x00\x00\x0D\x08\x70\x00\x00\x02\xC1\x02\x06\x22"_av);
}
...
RED_AUTO_TEST_CASE(TestSend_DT_TPDU)
{
    ...
    RED_CHECK(stream.get_produced_bytes() == "\x03\x00\x00\x0C\x02\xF0\x80"_av);
    RED_CHECK(payload.get_produced_bytes() == "\x12\x34\x56\x78\x9A"_av);
}
