...
/* nego sample */
static const uint8_t encodeHeaderSample[] = {
    /* as in 4.2.2 */
...
static const uint8_t encodeDataSample[] = {
...
static const uint32_t refImage[] = { /* 4.2.4.4 Inverse Color Conversion */
...
/* */
class CustomGdi : public gdi::NullGraphic {
public:
    CustomGdi() = default;

    void draw(RDPSetSurfaceCommand const & cmd, RDPSurfaceContent const &content) override {
#if 0
        FILE *f = fopen("/tmp/tile.data", "w");
        fwrite(content.data, 64 * 4, 64, f);
        fclose(f);
#endif
...
RED_AUTO_TEST_CASE(TestRemoteFx) {
    ...
    RDPSetSurfaceCommand cmd;
    cmd.destRect = Rect(0, 0, 64, 64);
    cmd.width = 64;
    cmd.height = 64;

    decoder.recv(negoStream, cmd, gdi);
