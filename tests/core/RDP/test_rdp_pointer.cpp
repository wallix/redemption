...
RED_AUTO_TEST_CASE(TestPointerVNC_BW)
{
    ...
    Pointer vnccursor = pointer_loader_vnc(BytesPerPixel{2}, 12, 19, 0, 0,
    ...
    RED_CHECK(vnccursor.get_monochrome_and_mask() == make_array_view(expected_mask, sizeof(expected_mask)));
...
RED_AUTO_TEST_CASE(TestPointerVNC_Color)
{
    ...
    Pointer vnccursor = pointer_loader_vnc(BytesPerPixel{2}, 23, 27, 0, 8,
        make_array_view(data), make_array_view(mask), 11, 31, 5, 63, 0, 31);
...
// //    cursor.emit(result);
//
// //    RED_CHECK(stream_to_avu8(result) == make_array_view(data, sizeof(data)));
// }
