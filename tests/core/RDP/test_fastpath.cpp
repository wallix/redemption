...
/* 0000 */ "\x10\x0e\x01\x0f\x62\x01\x0f\x20\x00\x08\xca\x00\x41\x03"         // ....b.. ....A.   |
    ...
    RED_CHECK(out_s.get_produced_bytes() == payload.first(2));
    RED_CHECK(out_payload.get_produced_bytes() == payload.from_offset(2));
}
...
/* 0000 */ "\x00\x07\x0a\x02\x00\x00\x00"                                     //.......
    ...
    RED_CHECK(SvrUpdPDU_s.get_produced_bytes() == payload.first(2)); // Server Fast-Path Update PDU (TS_FP_UPDATE_PDU)
    RED_CHECK(out_s.get_produced_bytes() == payload.from_offset(2)); // Fast-Path Update (TS_FP_UPDATE)
