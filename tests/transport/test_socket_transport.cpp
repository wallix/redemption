...
BOOST_AUTO_TEST_CASE(TestSocketTransport)
{
    ...
    {
        public:
        Server_status start(int incoming_sck) override { return START_WANT_STOP; }
    } dummy;
        ...
        default:
        {
            if (FD_ISSET(client_sck, &wfds)){
                if (client_trans && (data_sent == 0)){
                    ...
                    if (res != -1){
                        client_trans = new SocketTransport("Sender", client_sck, "127.0.0.1", 4444, 511);
                    }
                }
            }
            ...
            if (FD_ISSET(listener.sck, &rfds)){
                ...
                if (sck > 0){
                    ...
                    sck_trans[nb_recv_sck] = new SocketTransport("Reader", sck, "127.0.0.1", 4444, 511);
                    nb_recv_sck++;
