...
BOOST_AUTO_TEST_CASE(TestSplittedCapture)
{
    ...
    const int groupid = 0;
    {
        ...
        BOOST_CHECK_EQUAL(static_cast<unsigned>(3388) + 34, (unsigned)sq_outfilename_filesize(&wrm_seq, 1));
        sq_outfilename_unlink(&wrm_seq, 1);
        // Mem3Blt save state = 34 bytes
        BOOST_CHECK_EQUAL(static_cast<unsigned>(3364) + 34, (unsigned)sq_outfilename_filesize(&wrm_seq, 2));
