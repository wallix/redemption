...
BOOST_AUTO_TEST_CASE(TestSample0WRM)
{
    ...
    BOOST_CHECK_EQUAL(static_cast<unsigned>(1263943) + 34, (unsigned)sq_outfilename_filesize(&(out_wrm_trans.seq), 1));
    sq_outfilename_unlink(&(out_wrm_trans.seq), 1);
    // Mem3Blt save state = 34 bytes
    BOOST_CHECK_EQUAL(static_cast<unsigned>(360470) + 34, (unsigned)sq_outfilename_filesize(&(out_wrm_trans.seq), 2));
