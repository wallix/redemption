...
BOOST_AUTO_TEST_CASE(TestCapabilityMultiFragmentUpdateEmit)
{
    ...
    BOOST_CHECK_EQUAL(multifragmentupdate_caps.len, (uint16_t)RDP_CAPLEN_MULTIFRAGMENTUPDATE);
    ...
    BOOST_CHECK_EQUAL(multifragmentupdate_caps2.len, (uint16_t)RDP_CAPLEN_MULTIFRAGMENTUPDATE);

    BOOST_CHECK_EQUAL((uint16_t)CAPSETTYPE_MULTIFRAGMENTUPDATE, stream.in_uint16_le());
    BOOST_CHECK_EQUAL((uint16_t)RDP_CAPLEN_MULTIFRAGMENTUPDATE, stream.in_uint16_le());
    multifragmentupdate_caps2.recv(stream, RDP_CAPLEN_MULTIFRAGMENTUPDATE);
